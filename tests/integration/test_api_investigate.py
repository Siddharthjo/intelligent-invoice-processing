...
def test_investigate_endpoint_returns_trace_and_decision(
    ...
    with open(pdf_path, "rb") as f:
        upload_response = client.post("/invoices", files={"file": ("invoice.pdf", f, "application/pdf")})
    assert upload_response.status_code == 201
    invoice_id = upload_response.json()["id"]

    investigate_response = client.post(f"/invoices/{invoice_id}/investigate")
    ...
    get_response = client.get(f"/invoices/{invoice_id}/investigation")
    assert get_response.status_code == 200
    assert get_response.json()["agent_investigation_id"] == body["agent_investigation_id"]


def test_investigate_unknown_invoice_404s(client: TestClient):
    response = client.post(f"/invoices/{uuid.uuid4()}/investigate")
    assert response.status_code == 404


def test_get_investigation_404s_when_none_run_yet(client: TestClient, db_session: Session, tmp_path: Path):
    pdf_path = tmp_path / "invoice.pdf"
    ...
    response = client.get(f"/invoices/{invoice_id}/investigation")
    assert response.status_code == 404
