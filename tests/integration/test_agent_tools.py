...
def test_get_supplier_found_and_not_found(db_session: Session):
    ...
    context = ToolContext(session=db_session, invoice_id=None)
...
def test_check_duplicate_excludes_the_invoice_under_investigation(db_session: Session, tmp_path: Path):
    ...
    context = ToolContext(session=db_session, invoice_id=result.invoice_id)
    ...
    context_2 = ToolContext(session=db_session, invoice_id=second.invoice_id)
